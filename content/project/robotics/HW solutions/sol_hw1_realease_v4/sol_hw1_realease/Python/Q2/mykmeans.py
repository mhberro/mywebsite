...
if __name__ == "__main__":
    ...
    image = importImage(ImageName)
    numClusters = 3
    # cluster, clusterCenters, dist, timing, clusternum = mykmeans(image, numClusters)
    cluster, clusterCenters = mykmeans(image, numClusters)
    
    # restore pixels values
    imgout = np.zeros((cluster.shape[0], cluster.shape[1], 3))
    for ii in np.unique(cluster):
        imgout[cluster == ii] = clusterCenters[ii]/255
